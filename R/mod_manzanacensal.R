...
mod_manzanacensal_server <- function(id){
  moduleServer( id, function(input, output, session){
    ...
    manzana_censal_temp <- reactive({
      ...
      sf::st_drop_geometry(get_mc(point_selected()))
    })
    ...
    observeEvent(coords(), {
      ...
      if (input$results_manzanacensal$collapsed) { bs4Dash::updateBox("results_manzanacensal", action = "toggle")}
    ...
    output$map <- leaflet::renderLeaflet({
      ...
      tm <- tmap::tm_shape(
        manzanas |>
          filter(COMUNA == manzana_censal_temp()$COMUNA) |>
          mutate(
            color = case_when(
