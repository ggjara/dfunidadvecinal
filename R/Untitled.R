...
key <- "AIzaSyCH8rCawfkPxZfhb7Ndw_AtWSRykb5_hBI"
set_key(key = key)
#google_keys()
...
ui <- shiny::basicPage(
  ...
  div(
    ...
    ,shiny::verbatimTextOutput(outputId = "full_address")
    ,HTML(paste0(" <script>
...
server <- function(input, output) {
  ...
  my_address <- reactive({
    if(!is.null(input$jsValueAddressNumber)){
      if(length(grep(pattern = input$jsValueAddressNumber, x = input$jsValuePretty ))==0){
        ...
        final_address<- input$jsValueCoords
      }
      final_address
    }
  })
  ...
  output$full_address <- renderPrint({
